...
# GAT-MF + PM10 Adj on CPU — use when gpu-* queues are saturated.
#   FEATURE_CONFIGS=F1:F2:F3:F4:F5 SEQ_LEN=72 sbatch --export=ALL,FEATURE_CONFIGS,SEQ_LEN \
...
echo "CPU GAT-MF FEATURE_CONFIGS=$FEATURE_CONFIGS SEQ_LEN=$SEQ_LEN"
...
for cfg in "${CFG_ARR[@]}"; do
  ...
  DATA_DIR="data/training/zone_${ZONE}/gat_informer_mf_pm10graph/${cfg_trimmed}"
  if [[ "$SEQ_LEN" == "48" ]]; then
    OUT_DIR="results/gat_informer_mf_pm10graph/zone_${ZONE}/${cfg_trimmed}"
  else
    OUT_DIR="results/gat_informer_mf_pm10graph/zone_${ZONE}/seq_len_${SEQ_LEN}/${cfg_trimmed}"
  fi
  ...
  echo "===== TRAIN GAT-MF PM10 ${cfg_trimmed} seq_len=${SEQ_LEN} (CPU) ====="
...
echo "All GAT-MF PM10 CPU configs finished for SEQ_LEN=${SEQ_LEN}."
