...
dartboard_map = {0: '50-200',
                 ...
                 2: '50',
                 3: '25-100-250',
                 4: 'zone2_18'}  # MASTER zone-2 trivial rings (N=18)
...
class AirFormer(BaseModel):
    ...
    def forward(self, inputs, supports=None):
        ...
        '''
        # China AIR_* packs: categorical slots at channels 11:15 → AirEmbedding.
        # ZONE2 (and other continuous packs) must skip this path even when
        # input_dim >= 15 (F4/F5); otherwise continuous channels are cast to
        # Long and fed to Embedding tables sized for China categories.
        ds = str(getattr(self, "dataset", "") or "").upper()
        china_layout = ds.startswith("AIR_")
        if china_layout and inputs.size(-1) >= 15:
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Optional export for spatial interpretability (hour-of-day analysis).
        # Shape: [B, N, heads, num_sector] after reshape below.
        self.last_attn = attn.detach().reshape(
            B, N, self.num_heads, self.num_sector
        )
