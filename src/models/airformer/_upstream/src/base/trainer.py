...
from src.utils.metrics import masked_mae, masked_mse, masked_mae_peak_weighted
...
class BaseTrainer():
    def __init__(
            ...
            peak_weight_alpha: float = 0.0,
            peak_weight_thr: float = 40.0,
            peak_weight_thr2: float = 80.0,
            peak_asym_beta: float = 0.0,
            train_loss: str = "mae",
        ...
        self._aug = aug
        # Val/test keep plain loss; train may use peak weighting (MAE only).
        train_loss = (train_loss or "mae").lower()
        if train_loss not in ("mae", "mse"):
            raise ValueError(f"train_loss must be 'mae' or 'mse', got {train_loss!r}")
        self._train_loss = train_loss
        base_loss = masked_mse if train_loss == "mse" else masked_mae
        self._loss_fn = base_loss
        self._peak_weight_alpha = float(peak_weight_alpha or 0.0)
        self._peak_weight_thr = float(peak_weight_thr)
        self._peak_weight_thr2 = float(peak_weight_thr2)
        self._peak_asym_beta = float(peak_asym_beta or 0.0)
        if train_loss == "mse" and (
            self._peak_weight_alpha > 0 or self._peak_asym_beta > 0
        ):
            self._logger.warning(
                "Peak-weighted loss is MAE-only; using plain masked MSE for training."
            )
            self._train_loss_fn = masked_mse
        elif self._peak_weight_alpha > 0 or self._peak_asym_beta > 0:
            alpha, thr, thr2, beta = (
                self._peak_weight_alpha,
                self._peak_weight_thr,
                self._peak_weight_thr2,
                self._peak_asym_beta,
            )

            def _train_loss(preds, labels, null_val=0.0):
                return masked_mae_peak_weighted(
                    preds,
                    labels,
                    null_val,
                    peak_alpha=alpha,
            ...
            self._train_loss_fn = _train_loss
            self._logger.info(
                "Peak-weighted train loss: alpha={} thr={} thr2={} asym_beta={}".format(
                    alpha, thr, thr2, beta
                )
            )
        else:
            self._train_loss_fn = base_loss
        self._logger.info("Training loss: %s", train_loss.upper())
    ...
    def train_batch(self, X, label, iter):
        ...
        loss = self._train_loss_fn(pred, label, 0.0)
        loss.backward()
    ...
    def test(self, epoch, mode='test'):
        ...
        # Full-horizon hourly metrics (MASTER / ZONE2 use horizon=24 as h+1..h+24)
        mae_h, rmse_h = mc.compute_all_metrics(preds, labels, 0.0)
        pred_ma = preds.mean(dim=1, keepdim=True)
        label_ma = labels.mean(dim=1, keepdim=True)
        mae_ma, rmse_ma = mc.compute_all_metrics(pred_ma, label_ma, 0.0)
        print('Hourly Test MAE: {:.4f}, Test RMSE: {:.4f}'.format(mae_h, rmse_h))
        print('MA24   Test MAE: {:.4f}, Test RMSE: {:.4f}'.format(mae_ma, rmse_ma))

        rows = [
            {'Time': 'hourly', 'Test MAE': mae_h, 'Test RMSE': rmse_h},
            {'Time': 'ma24', 'Test MAE': mae_ma, 'Test RMSE': rmse_ma},
        ]
        ...
        if self.model.horizon == 24:
            ...
            for i in range(0, self.model.horizon, 8):
                metrics = mc.compute_all_metrics(
                    preds[:, i: i + 8], labels[:, i: i + 8], 0.0)
                amae_day.append(metrics[0])
                armse_day.append(metrics[1])
            rows.extend([
                {'Time': '0-7h', 'Test MAE': amae_day[0], 'Test RMSE': armse_day[0]},
                {'Time': '8-15h', 'Test MAE': amae_day[1], 'Test RMSE': armse_day[1]},
                {'Time': '16-23h', 'Test MAE': amae_day[2], 'Test RMSE': armse_day[2]},
            ])

        # Sudden-change mask is AIR_TINY-specific; skip when unavailable
        air_tiny_mask = os.path.join(
            './data/AIR_TINY', 'mask_sudden_change_75_20.pth')
        if os.path.exists(air_tiny_mask) or os.path.isdir('./data/AIR_TINY'):
            try:
                mask_sudden_change = mc.sudden_changes_mask(
                    labels, datapath='./data/AIR_TINY', null_val=0.0,
                    threshold_start=75, threshold_change=20)
                sc_mae, sc_rmse = mc.compute_sudden_change(
                    mask_sudden_change, preds, labels, null_value=0.0)
                rows.append({
                    'Time': 'SuddenChange',
                    'Test MAE': sc_mae,
                    'Test RMSE': sc_rmse,
                })
                print('Sudden Changes MAE: {:.4f},  RMSE: {:.4f}'.format(
                    sc_mae, sc_rmse))
            except Exception as exc:
                print('Skipping sudden-change metrics:', exc)

        results = pd.DataFrame(rows)
        results.to_csv(
            os.path.join(self.save_path, 'metrics_{}.csv'.format(self._n_exp)),
            index=False)

        # Persist arrays for MASTER post-processing (raw units already inverted)
        np.save(os.path.join(self.save_path, 'test_preds.npy'), preds.numpy())
        np.save(os.path.join(self.save_path, 'test_labels.npy'), labels.numpy())
